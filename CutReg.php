...
<section class="w3l-contact-11 py-5" id="contact">
    <div class="container py-lg-5 py-md-4 py-2">
        <h3 class="title-style text-center mb-lg-5 mb-4">Customer <span>Sign Up</span></h3>
        <div class="form-inner-cont mx-auto" style="max-width:800px">
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($psw == $cpsw) {
                    ...
                    $sel = "SELECT COUNT(*) AS COUNT FROM `customer` WHERE `contact`='$contact' OR `email`='$email'";
                    $res = mysqli_query($con, $sel);
                    $fetch = mysqli_fetch_array($res);
                    if ($fetch['COUNT'] > 0) {
                        echo '<script>alert("Already Registered")</script>';
                    } else {
                        $qry = "INSERT INTO `customer`(`name`,`address`,`location`,`contact`,`email`,`proof`,`img`)VALUES('$name','$address','$district','$contact','$email','$file','$file1')";
                        $exe = mysqli_query($con, $qry);
                        if ($exe) {
                            $qry2 = "INSERT INTO `login`(`uid`,`uname`,`psw`,`type`)VALUES((SELECT MAX(`uid`) FROM `customer`),'$email','$psw','Customer')";
