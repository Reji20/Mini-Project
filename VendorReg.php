...
<section class="w3l-contact-11 py-5" id="contact">
    <div class="container py-lg-5 py-md-4 py-2">
        <h3 class="title-style text-center mb-lg-5 mb-4">Vendor <span>Sign Up</span></h3>
        <div class="form-inner-cont mx-auto" style="max-width:800px">
            <form method="post" class="signin-form" enctype="multipart/form-data">
                <div class="row align-form-map">
                    ...
                    <div class="col-sm-6 form-input">
                        <input type="password" name="cpsw" placeholder="Password" class="contact-input">
                    </div>
                    <div class="form-input">
                        <input type="text" name="regnum" placeholder="Registration Number" class="contact-input">
                    </div>
                    <div class="col-sm-6 form-input">
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($psw == $cpsw) {
                    ...
                    $sel = "SELECT COUNT(*) AS COUNT FROM `vendor` WHERE `contact`='$contact' OR `email`='$email'";
                    $res = mysqli_query($con, $sel);
                    $fetch = mysqli_fetch_array($res);
                    if ($fetch['COUNT'] > 0) {
                        echo '<script>alert("Already Registered")</script>';
                    } else {
                        $qry = "INSERT INTO `vendor`(`name`,`address`,`location`,`contact`,`email`,`proof`,`img`,`renum`)VALUES('$name','$address','$district','$contact','$email','$file','$file1','$regnum')";
                        // echo $qry;
                        $exe = mysqli_query($con, $qry);
                        if ($exe) {
                            $qry2 = "INSERT INTO `login`(`uid`,`uname`,`psw`,`type`)VALUES((SELECT MAX(`uid`) FROM `vendor`),'$email','$psw','Vendor')";
